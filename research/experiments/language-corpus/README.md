...
Reviewed: 2026-09-23. The frozen screening metadata remains unchanged. The [Arabic](arabic-audio-pilot.md) and [Hindi](hindi-audio-pilot.md) calibration clips have since been acquired, extracted and decoded with measured hashes; recognition remains open. The [Spanish archive](spanish-archive-transfer-pilot.md) and [selected original](spanish-audio-pilot.md) have passed verified transfer and extraction; bounded decoding remains open. No inference or paid request was used in these corpus pilots. The full archives and [bounded Parquet route](parquet-acquisition-review.md) exceeded the initial 10 GiB allowance, which the user has since raised to 100 GB. The [acquisition route decision](acquisition-route-decision.md) selects the revision-pinned original archives for their publisher whole-object hashes. A one-clip [Dataset Viewer filter pilot](../../30-language-pipeline-evaluation.md#frozen-fleurs-screening-selection-and-acquisition-gate-2026-09-22) showed individual access but did not establish all 112 identities and has weaker immutable provenance. The [alternative-route review](alternative-route-review.md) found a smaller test-only archive path but no complete replacement calibration set. This directory retains the frozen selection and derived metadata; repeat network probes in an ignored working copy so transient files are not committed.
...
The [pinned card acquisition](card-provenance-pilot.md) retained and hashed the revision's 385,614-byte card on 2026-09-23. A separate bounded request retained the official CC BY 4.0 legal text. The [pilot attribution packet](fleurs-subset-attribution.md) records the supplied credits, license evidence, and selected identities. Each remaining archive still needs its own notice review before use. The card is provenance evidence, not a selected recording.

The [Arabic](arabic-reference-pilot.md), [Hindi](hindi-reference-pilot.md), and [Spanish](spanish-reference-pilot.md) calibration reference pilots each retain and validate one pinned train TSV and selected row for evaluator-only use. The separately acquired [Arabic](arabic-audio-pilot.md) and [Hindi](hindi-audio-pilot.md) audio have reached bounded decoding, with no speech recognition result. The selected Spanish original has passed extraction but awaits bounded decoding; holdout references remain unrequested.

The first selected Arabic, Hindi, and Spanish originals have measured sizes, encodings, and content hashes in their pilot records. Most of the remaining selected audio is still unmeasured. Derived storage estimates from the selected sample counts are 41,130,688 bytes for mono PCM16 WAV with a 44-byte header, or 82,256,448 bytes for float32 with that simplified header. These are mathematical estimates for all 112 simplified output files, not measured server objects or download reservations; real headers and formats can differ.

`archive-acquisition.json` records every relevant full train/test archive URL, declared byte size, Git blob ID and LFS SHA-256 from the pinned official Hub tree API. The first Arabic train archive now has an independently matching local hash; the other fifteen remain metadata declarations. Sixteen full archives total **16,113,568,959 bytes, about 15.01 GiB**, before models and metadata. They exceeded the initial 10 GiB allowance but fit under the current 100 GB ceiling. Train archives alone are 12,920,048,425 bytes; test archives are 3,193,520,534 bytes. Each further archive needs its own bounded acquisition and selective-extraction review. The sixteen TSV bodies total 17,730,888 UTF-8 bytes per full metadata pass; planning performed repeated metadata requests before the single Arabic audio GET.
...
The earlier metadata reads used URL-encoded parameters, explicit subset/split discovery, zero-based offsets, at most 100 rows per row-like request, and pagination/partial checks. That metadata-only work used no account token, Python, inference, audio download or paid request. The failed Viewer access was not bypassed with unbounded parquet fetching. The later [Arabic](arabic-audio-pilot.md), [Hindi](hindi-audio-pilot.md), and [Spanish](spanish-archive-transfer-pilot.md) archive GETs are recorded separately.

Local validation checks JSON parsing, 112 unique asset locators, exact 4/10 per-language counts, duration bounds, all required subsets, reference hash format, global split separation, and archive count/size. Promoted scripts and data were compared byte for byte with the reviewed scratch files before staging; Git normalizes text line endings. This checks the metadata plan. The Arabic and Hindi pilots check their archives, original WAVs, and decoded worker hashes; the Spanish pilot checks its archive and original WAV. Actual reference fidelity, acceleration, and language quality remain unmeasured.
