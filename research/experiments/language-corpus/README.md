...
The [Arabic calibration reference pilot](arabic-reference-pilot.md) retains and validates one pinned `ar_eg` train TSV and selected row for evaluator-only use. The separately [acquired Arabic audio](arabic-audio-pilot.md) has reached bounded decoding, not speech recognition. Hindi, Spanish and holdout references remain unrequested.
