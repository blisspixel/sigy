...
> **Development preview.** Radio and podcast discovery, listening, recording, schedules, and a terminal explorer work today on Windows. Speech recognition, translation, and topic monitoring are being built now. See [what works today](#what-works-today) and the [progress record](docs/development/progress.md).
...
| Speech recognition | In progress: first local calibration runs; not yet available in the app |
...
Local speech recognition is the current focus. A pinned multilingual recognizer is running in bounded calibration tests on Arabic, Hindi, and Spanish reference clips, and is being wired into the service. English translation and topic monitoring follow. The [roadmap](ROADMAP.md#whats-next) explains the order and the reasons, and the [language pipeline plan](docs/development/language-pipeline.md) defines how quality is measured.
