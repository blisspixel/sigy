...
**Status: early Rust implementation. Radio Browser refresh, cached search and favorites, an explicit directory click, finite service-owned audio recording, retained-file playback, playlist resolution, a finite listen of one direct audio revision, finite HLS media-playlist recording, explicit ICY observations on `record start --icy`, decoder validation, metadata export, rolling storage policy, and local podcast subscribe work through the CLI. On this Windows host, with FFmpeg 9.0.1, one local session decoded direct WAV, MP3, AAC (ADTS served as audio/aac), FLAC, and Ogg Vorbis. The same session decoded WAV from one accepted playlist entry behind one same-origin redirect. The fixture contacts only 127.0.0.1. One smoke session is not a support matrix. Local Windows tests and a live directory check support this increment. The [roadmap build order](ROADMAP.md#build-order) is the path to the first complete release. `sigy tui` is the list explorer. It uses Ratatui 0.30.2 with the Termina 0.3.3 backend. On this Windows host it was inspected in Windows Terminal 1.24.11911.0 at 80x24 with the Consolas font. Selection does not start audio, capture, refresh, or a click, and quit does not stop the service. `podcast subscribe` stores one local feed subscription. It does not resolve DNS or start a capture. `podcast refresh` reads one RSS 2.0 document on the shared acquirer and lists episodes offline. It does not download enclosures, transcripts, or chapters. `podcast download` fetches one stored enclosure through the recording path and reserves 512 MiB and 30 minutes before connecting. `listen file` plays that retained recording. The episode has no live edge, so `listen source` does not open the enclosure. Subscribe and refresh still do not download or play. Unsubscribe stops future polls and deletes nothing. The globe and model processing remain to build.**
...
Internet radio and podcasts are the first source priorities, before physical radios. Podcast subscribe, feed refresh, one enclosure download, retained playback, and one explicit publisher transcript or chapter fetch reuse the recording and evidence controls. Broader RSS and Atom text analysis extends that path later.
...
[Favorites](docs/decisions/0008-radio-favorites.md) work offline and survive catalog updates and service restarts. Removing a favorite preserves the station, registered sources and recordings.

Subscribe to one podcast feed without fetching it:

```text
cargo run --locked -p sigy -- --data-dir PATH_TO_LIBRARY podcast subscribe show:v1 --url https://show.example/feed.xml
cargo run --locked -p sigy -- --data-dir PATH_TO_LIBRARY podcast show show:v1
cargo run --locked -p sigy -- --data-dir PATH_TO_LIBRARY podcast unsubscribe show:v1
```

The feed URL, network scope, address pin, and redirect policy are immutable. Reuse of the same id cannot change them. `--pin-address` and `--redirects` match source registration, and the default redirect policy is deny. Subscribe does not resolve DNS, download the document, register an audio source, or start a capture. Unsubscribe stops future polls and deletes nothing, including the subscription row, favorites, sources, and recordings. Ordinary output shows the origin and omits the feed path and query.

Refresh one stored feed through the running service. The read is one RSS 2.0 document on the shared acquirer. A compressed body is at most 2 MiB, the decoded document is at most 8 MiB, and the expansion ratio is at most 16. At most 500 items are committed. Further items mark the snapshot truncated. A failed document leaves the last good snapshot. Omission from a later snapshot does not delete an episode. Transcript and chapter URLs are stored and not fetched. A live item is counted and not opened. Episode titles are not identity. Listing episodes works offline. This does not download enclosure media.

```text
cargo run --locked -p sigy -- --data-dir PATH_TO_LIBRARY podcast refresh show:v1 --id feed:v1
cargo run --locked -p sigy -- --data-dir PATH_TO_LIBRARY podcast refresh-status feed:v1
cargo run --locked -p sigy -- --data-dir PATH_TO_LIBRARY podcast episodes show:v1
```

Download one stored enclosure only when asked. The service registers an immutable audio revision for that URL under the subscription's network grant, reserves 512 MiB and 30 minutes before connecting, and publishes a clean ended file through the existing decoder and hash. Radio attempts stay 15 minutes and 256 MiB. A declared length above 512 MiB is rejected before connect. Replay of the same recording id does not download again.

```text
cargo run --locked -p sigy -- --data-dir PATH_TO_LIBRARY podcast download show:v1 --episode EPISODE_ID --id episode:v1 --revision enc:v1
```

`EPISODE_ID` comes from `podcast episodes`. The download command does not fetch transcripts or chapters. `podcast text` fetches one of those stored documents when asked. The result is unverified publisher text, and its cue times are not media time. Refresh still does not fetch the asset. Play the retained file with the existing player. The episode has no live edge, so `listen source` on that revision fails before a request. Subscribe and refresh do not start playback. The tested recording stays temporary under the default 14-day and 50 GB policy, and playback does not change charged bytes.
...
See [local podcast subscriptions](docs/decisions/0017-local-podcast-subscriptions.md), [RSS feed refresh](docs/decisions/0018-rss-feed-refresh.md), [episode enclosure download](docs/decisions/0019-episode-enclosure.md), [retained episode playback](docs/decisions/0020-retained-episode-playback.md), and [publisher text](docs/decisions/0022-publisher-text.md). Catalog schema is v16 and local IPC is v16. Stop an older service with its existing binary before updating, then restart it.
...
Default retention is 14 days with a 50 GB total managed-media quota. A temporary recording may stay while it is processed. `record keep ID` and `record archive ID` keep it, and those bytes still count toward quota. Archive does not create a backup. New recordings fail clearly if protected media fills the allowance. `record temporary ID` restores rolling retention. `record processed ID --receipt RECEIPT_ID` records that processing finished. It does not run analysis. The service sweep then deletes that temporary file. The same sweep deletes other temporary recordings older than the retention window. Quota pressure removes the oldest temporary recordings sooner. `dvr prune` runs that same reclaim. `record delete ID` deletes inactive media even when protected and keeps the catalog history.

This initial recording profile accepts direct audio responses, explicitly permitted redirects, one finite HLS media playlist, and explicit ICY metadata on `record start --icy`, with up to two active attempts, 15 minutes and 256 MiB per attempt. On Windows, with FFmpeg 9.0.1, one local session decoded direct WAV, MP3, AAC (ADTS served as `audio/aac`), FLAC, and Ogg Vorbis. The same session decoded WAV from one accepted playlist entry behind one same-origin redirect and played that revision to `--destination null`. The fixture contacts only 127.0.0.1. One smoke session is not a support matrix. See [decoded formats](docs/decisions/0014-decoded-formats.md). Playlist media types still fail on `record start`. A listen, an HLS recording, and `record start` without `--icy` still reject an `icy-metaint` response before writing audio. Local podcast subscribe does not download an enclosure. Feed refresh stores episode metadata and does not download enclosure bytes. `podcast download` fetches one enclosure, reserves 512 MiB and 30 minutes before connect, and leaves radio attempts at 15 minutes and 256 MiB. A body that hits the ceiling without a clean end is not playable. A retained episode plays with `listen file`. `listen source` does not open that enclosure. Continuous segmented DVR is not yet qualified. Failed/partial bytes retain their reservation and are not offered as playable media. The decoder is supervised, but aggregate native-memory sandboxing and physical power-loss durability remain open. See [recording and retention](docs/decisions/0005-recording-and-retention.md), [measured recording intervals](docs/decisions/0023-recording-intervals.md), and the extensible [recording metadata design](docs/design/recording-metadata.md).
