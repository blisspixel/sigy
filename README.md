...
`EPISODE_ID` comes from `podcast episodes`. The command does not fetch transcripts or chapters. See [local podcast subscriptions](docs/decisions/0017-local-podcast-subscriptions.md), [RSS feed refresh](docs/decisions/0018-rss-feed-refresh.md), and [episode enclosure download](docs/decisions/0019-episode-enclosure.md). Catalog schema is v14 and local IPC is v15. Stop an older service with its existing binary before updating, then restart it.
