...
The feed URL, network scope, address pin, and redirect policy are immutable. Reuse of the same id cannot change them. `--pin-address` and `--redirects` match source registration, and the default redirect policy is deny. Subscribe does not resolve DNS, download the document, register an audio source, or start a capture. Unsubscribe stops future polls and deletes nothing, including the subscription row, favorites, sources, and recordings. Ordinary output shows the origin and omits the feed path and query. Feed refresh and episode download are not implemented. See [local podcast subscriptions](docs/decisions/0017-local-podcast-subscriptions.md). Catalog schema is v12 and local IPC is v13. Stop an older service with its existing binary before updating, then restart it.
