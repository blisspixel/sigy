...
| R-21 | C-01 | Focus, resizing, Unicode, and keyboard navigation preserve user work | Terminal interaction fixtures, rendered inspection and keyboard task walkthroughs; user feedback when available |
...
| R-36 | C-23, C-37 | Exploration/workbench journeys are engaging and let users inspect inputs and intermediate results, explain displayed activity and identify uncertainty alongside active collection | Newcomer explanation/replay walkthroughs, rendered interaction review, keyboard/reduced-motion layouts, synthetic-origin labels, ambiguity and resource contention; label untested user enjoyment honestly |
...
| R-39 | C-24 | Supported everyday workflows are discoverable and usable without specialist prerequisite knowledge | Task-based walkthroughs, preset/error-recovery review, comprehension of results and limits, advanced-control discoverability; user sessions improve evidence when available |
...
| R-51 | C-33 | First-release podcast ingestion and supplied resources preserve bounded, idempotent, revision-bound evidence; later syndicated text analysis extends that contract | Feed/episode revision and duplicate fixtures, XML/network boundaries, partial downloads and supplied-transcript provenance for the first release; later multilingual text, queue fairness and cost-limit evaluation |
...
R-46 through R-50 refine the first-release radio and terminal experience, with RF-specific visualizations qualified alongside hardware. R-51 requires the podcast ingestion, enclosure and supplied-resource checks for the first release. Broader syndicated text processing is qualified in stage 9 on those shared contracts.
