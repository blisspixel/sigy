...
Amended on 2026-09-24 by [live HLS and master playlist candidates](0042-live-hls.md): an HLS master playlist now resolves to its variants and audio renditions with declared bandwidth, codecs, and an audio-only flag, and accepting one registers it like an entry. No variant is fetched during resolve. An HLS media playlist still fails and stores no candidates. A directory HLS flag no longer fails the resolve. Catalog schema and local IPC are v30.
