...
- Amended 2026-09-24: the [Vulkan measurement](../../research/experiments/local-mt/vulkan-780m.md) found that llama.cpp offloads every layer to a GPU backend present in the runtime folder without being asked. The worker now passes `-dev none -ngl 0`, confirmed on the Vulkan package to keep all 33 Hy-MT2 layers on the CPU, and the test stand-in refuses to run without those flags. A GPU profile will be a separate profile kind with its own identity and calibration.
