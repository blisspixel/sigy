...
`NO_COLOR` selects monochrome. `TERM=dumb` does too. `SIGY_REDUCED_MOTION=1` and `SIGY_LINEAR=1` select those modes, as do the matching flags. `--inspect` draws one frame, writes a size report, and exits.

Color is semantic and uses the terminal's own ANSI palette. Cyan marks the title, the current workspace, search, and the selected row. Green marks a live connection and a succeeded or completed state. Yellow marks a favorite, a running or stopping state, and a stale count. Red marks a disconnect or a failure. Muted gray marks unavailable workspaces and identifiers. Help text stays uncolored. The words remain when color is removed. Linear order and monochrome draw no semantic color. Focus in color is bold. Focus in monochrome is reversed. There is still no animation.

## Evidence

State tests cover focus, ordinary search editing, paste that contains `q`, selection that starts nothing, a stale search, a stale favorite, disconnect that keeps the last snapshot and blocks a favorite, and quit while a recording is `running`. Layout tests at 80x24 keep search, selected source, directory health, directory languages, playback, and help, and they keep the partial cache, refresh outcome, observation age, favorites, recording, and quota. A 10x4 terminal renders a recovery prompt. Linear order is top to bottom. Monochrome uses no color. Color is used only when monochrome and linear order are off. On the Ratatui test backend, succeeded health is green, a favorite is yellow, the title is cyan, a disconnect is red, and the help line has no color. Unavailable workspaces are visible.
