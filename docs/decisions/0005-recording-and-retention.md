...
Schema v4 adds one DVR policy and recording records. Defaults are 14 days and 50,000,000,000 media bytes, with a 256 MiB free-space floor. Configuring a trusted installed decoder is explicit. A temporary recording may remain while it is processed. The service sweep deletes it after a processing receipt, after it passes the retention age, or oldest-first when a new reservation needs quota. Kept and archived media are exempt from that sweep and still consume quota. `record processed` records the receipt and does not run analysis. Archive is a retention designation, not a second copy or backup.
