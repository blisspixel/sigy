...
The current protocol is v3: length-prefixed JSON, strict request fields, 16 KiB requests, 256 KiB responses, 32 clients, a bounded actor queue, and five-second client deadlines. Status, budget limits, source registration/list/inspection and stop operations are exposed. Source operations were added on 2026-09-21 under [decision 0004](0004-source-authority-and-http.md). A lost reply does not roll back a committed mutation; clients do not automatically retry mutations. A protocol mismatch fails explicitly rather than guessing compatibility.
