...
A local fixture publishes a WAV equal to the audio with the metadata removed. The published SHA-256 matches that audio. The export contains one hostile title and stream URL, the source name is unchanged, and the fixture records no request for that URL. The same server without `--icy` fails before publication. This does not qualify public ICY stations or non-UTF-8 metadata.
