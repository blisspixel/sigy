...
The proposed [scaling architecture](design/scaling-architecture.md) keeps one task contract from a laptop to many workers, containers or serverless executors. [Live HLS](decisions/0042-live-hls.md) records live media playlists and lists master variants for explicit acceptance. The [local translation worker](decisions/0041-local-translation-worker.md) translates recognized cues into English one contained process per cue. [Exact provider pricing](decisions/0037-exact-provider-pricing.md) parses per-token prices exactly and computes a rounded-up worst-case liability; nothing dispatches yet. [Provider configuration and billing faults](decisions/0040-provider-configuration-and-billing-faults.md) stores immutable routes with secret names only and dated price snapshots, and proves the billing-fault fixtures against a fake transport; dispatch stays unavailable. [Globe geometry](decisions/0038-globe-geometry.md) adds orthographic and equirectangular projection and a geometric solar terminator from one explicit instant. Nothing renders the globe yet.
