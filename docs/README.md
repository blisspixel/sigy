...
[Exact provider pricing](decisions/0037-exact-provider-pricing.md) parses per-token prices exactly and computes a rounded-up worst-case liability; nothing dispatches yet. [Globe geometry](decisions/0038-globe-geometry.md) adds orthographic and equirectangular projection and a geometric solar terminator from one explicit instant. Nothing renders the globe yet.
