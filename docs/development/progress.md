...
The preceding redirect checkpoint is commit `5011e1d`. The favorites increment passes **92 ordinary tests plus 4 explicit native-media tests** on Windows x86_64, with the intermittent media-fixture caveat below. `./scripts/verify.ps1` passed native source hashes, formatting, workspace tests, warnings-denied Clippy, build and dependency auditing. The advisory check reviewed 239 dependency entries against 1,258 loaded advisories without findings. Favorites add no packages or lockfile changes.
...
Catalog schema is v7 and IPC is v7; recording envelope remains v2. Stop older controllers before replacing binaries. Existing schema versions migrate transactionally; automatic backups and power-loss recovery still require qualification. The single library owner retains the lock through workers and catalog operations. Read [controller](../decisions/0002-local-controller.md), [journal](../decisions/0003-capture-journal.md), [source authority](../decisions/0004-source-authority-and-http.md), [recording/retention](../decisions/0005-recording-and-retention.md), [directory refresh](../decisions/0006-radio-discovery.md), [authorized redirects](../decisions/0007-authorized-redirects.md) and [favorites](../decisions/0008-radio-favorites.md) decisions.
...
Connect the implemented catalog, favorites and recordings to integrated playback and bounded playlist handling through the existing checked redirect path. Resolve click-reporting requirements separately from metadata reads and qualify public stream compatibility. Add the first rendered terminal explorer through existing operations; keep partial-cache coverage and stale observations visible.

Optional [network routing](../design/network-routing.md) is researched in D-34/R-58: reuse the existing client, account for proxy-side DNS and distinct peer evidence, and qualify no direct fallback. Personal-machine and server operation share the service; remote control and external VPN compatibility remain qualification tasks. This does not replace the internet-radio/podcast work sequence.
